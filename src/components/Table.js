...
const FavButton = ({ data }) => {
  const { setCoin, Removecoin, allcoins } = useContext(FavouriteContext);
  const Ehandler = (coinid) => {
  ...
  return (
    ...
    >
      <svg
        className={`${
          allcoins.includes(data)
            ? `mr-2 fill-green`
            : `fill-gray-100 mr-2 hover:fill-grecyan`
        }`}
        width="30"
        height="30"
        viewBox="0 0 30 30"
...
const Table = () => {
  let { cryptoData, currency } = useContext(Cryptodata);
